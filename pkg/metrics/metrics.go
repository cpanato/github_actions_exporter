...
var (
	workflowJobHistogramVec = prometheus.NewHistogramVec(
	...
	workflowJobDurationCounter = prometheus.NewCounterVec(
	...
	workflowJobStatusCounter = prometheus.NewCounterVec(
		prometheus.CounterOpts{
	...
	workflowRunHistogramVec = prometheus.NewHistogramVec(
		prometheus.HistogramOpts{
	...
	workflowRunDurationGauge = prometheus.NewGaugeVec(
		...
		[]string{"org", "repo", "run_id", "run_attempt", "run_number", "status", "conclusion", "workflow_name"},
	)

	totalMinutesUsedActions = prometheus.NewGaugeVec(
		prometheus.GaugeOpts{
	...
	includedMinutesUsedActions = prometheus.NewGaugeVec(
		prometheus.GaugeOpts{
	...
	totalPaidMinutesActions = prometheus.NewGaugeVec(
		prometheus.GaugeOpts{
	...
	totalMinutesUsedUbuntuActions = prometheus.NewGaugeVec(
		prometheus.GaugeOpts{
	...
	totalMinutesUsedMacOSActions = prometheus.NewGaugeVec(
		prometheus.GaugeOpts{
	...
	totalMinutesUsedWindowsActions = prometheus.NewGaugeVec(
		prometheus.GaugeOpts{
	...
	totalMinutesUsedByHostTypeActions = prometheus.NewGaugeVec(
		prometheus.GaugeOpts{
	...
	runnersGauge = prometheus.NewGaugeVec(
		...
		[]string{"repo", "id", "name", "os", "status", "busy"},
	)
)
...
type WorkflowObserver interface {
	...
	ObserveWorkflowRunDuration(org, repo, status, conclusion, workflow string, seconds float64)
	CountWorkflowRunDuration(org, repo, runID, runAttempt, runNumber, status, conclusion, workflow string, seconds float64)
}
...
func (o *PrometheusObserver) ObserveWorkflowJobDuration(org, repo, state, runnerGroup string, seconds float64) {
	workflowJobHistogramVec.WithLabelValues(org, repo, state, runnerGroup).Observe(seconds)
}
...
func (o *PrometheusObserver) ObserveWorkflowRunDuration(org, repo, status, conclusion, workflowName string, seconds float64) {
	workflowRunHistogramVec.WithLabelValues(org, repo, status, conclusion, workflowName).Observe(seconds)
}

func (o *PrometheusObserver) CountWorkflowRunDuration(org, repo, runID, runAttempt, runNumber, status, conclusion, workflowName string, seconds float64) {
	workflowRunDurationGauge.WithLabelValues(org, repo, runID, runAttempt, runNumber, status, conclusion, workflowName).Set(seconds)
}
